...
The runner ships in this plugin, two levels above this skill's base directory:
`<plugin root>/scripts/damira.py`. It runs locally and makes no API call.
...
```bash
python3 "<plugin root>/scripts/damira.py" lab preflight
```
...
```bash
python3 "<plugin root>/scripts/damira.py" lab validate labs/<lab>.clab.yml
```
...
```bash
python3 "<plugin root>/scripts/damira.py" lab run labs/<lab>.clab.yml \
