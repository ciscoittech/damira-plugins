...
version-constraint pattern, auth env vars and known quirks. Call
`damira_search_vendor_docs` for resource names and arguments you are not sure of —
...
```bash
python3 "<plugin root>/scripts/damira.py" validate terraform/{name}/ --skill generate-terraform --host claude-code
```
