...
`damira init` wrote to CLAUDE.md names the servers it found. If there is no
...
**Delegate this loop to the `damira:damira-fixer` agent** (it runs on Haiku). Give it the
path, `--skill source-of-truth-change`, and `<plugin root>`. It fixes only what the
findings name and stops after 3 rounds. If the agent isn't available, run it yourself:

```bash
python3 "<plugin root>/scripts/damira.py" validate <path> --skill source-of-truth-change --host claude-code
```
