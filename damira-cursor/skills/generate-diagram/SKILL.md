...
The Damira scripts parse the configs into a spec (`topology.json`) and draw it. You
...
```bash
~/.damira/bin/damira diagram build configs/ -o documents/<id>/topology.json
```
...
```bash
~/.damira/bin/damira diagram render documents/<id>/topology.json
```
...
1. Give the paths: `topology.svg` for documents, `index.html` for viewing (the diagram
   plus device and link tables; self-contained). Open it in Cursor's browser.
2. Show the Mermaid inline in a ```mermaid block, so the engineer sees it in chat and can
   paste it into a wiki or a ticket.
3. **If this Cursor build can show a canvas**, build one from `topology.json`, not from
   the SVG. Keep it local:
   - a stats row: devices, links, external nodes, inferred (subnet or bgp) links
   - the diagram, nodes by tier top to bottom, dashed for `bgp` edges and external nodes
   - a link table: both ends, interfaces, subnet, source

   No canvas available? The preview covers it. Don't mention the canvas.

Need a copy to put in a ticket or send outside the team? Render a redacted one; never
share the original preview:
...
the same in every file. Show the user the redaction counts and ask them to check
`documents/<id>/share/index.html` for anything left.
